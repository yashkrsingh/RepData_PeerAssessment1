...
ggplot(data = test, aes(test$steps)) + geom_histogram(bins = 20,col = "black", fill = "blue", alpha = 0.7) + labs(x = "Number of steps", y = "Frequency", title = "Number of Steps Taken Each Day")
m1 <- aggregate(steps ~ date, omit, mean)
m2 <- aggregate(steps ~ date, omit, median)
